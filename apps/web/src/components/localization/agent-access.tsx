...
export function AgentAccess({ projectId }: { projectId: string }) {
	...
	const effectiveScopes = isReviewer
		...
		: selectedScopes.filter(
				(scope) =>
					...
					(project?.type === "basic" || scope !== "strings-write"),
			);
			...
			<div className="flex flex-col gap-4">
				{project ? (
					<AgentReviewPolicyCard
						...
						isOwner={isOwner}
						onChange={(enabled) =>
							setAgentReviewPolicy({ projectId: convexProjectId, enabled })
						}
					/>
				) : null}
				{isOwner ? (
						...
						<CardContent>
							<form onSubmit={submit}>
								<FieldGroup>
									...
									{!isReviewer && project?.type === "basic" ? (
										<Field orientation="horizontal">
											<Checkbox
												id="strings-write-token"
												checked={selectedScopes.includes("strings-write")}
												disabled={isCreating}
												onCheckedChange={(checked) =>
													toggleScope("strings-write", checked === true)
												}
											/>
											<FieldContent>
												<FieldLabel htmlFor="strings-write-token">
													Allow string creation
												</FieldLabel>
												<FieldDescription>
													Add source text, optional names, context and character
													limits. Translations still use proposals and review.
												</FieldDescription>
											</FieldContent>
										</Field>
									) : null}
