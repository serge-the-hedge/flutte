...
Canonical request, response, authorization, and lifecycle contracts for the
project-scoped `/api/agent/v1` API. For task procedures, load the appropriate
[agent skill](https://github.com/serge-the-hedge/flutte/tree/main/agent-kit). Consult individual endpoint sections as needed.
Agents submit immutable candidates; only authorized review applies their values.
Retired catalog addresses return migration errors.
...
[evaluation](https://github.com/serge-the-hedge/flutte/blob/main/reports/native-convex-search-evaluation-2026-09-07.md).
...
These writes update the active Dictionary directly. Preserve other Locale
renderings when replacing a complete term. Authorization comes from an explicit
`dictionary-write` grant; translation and reviewer tokens do not acquire it.
An unchanged write at the current revision creates no extra history. If a
response is lost or the server returns `STALE_BASIS`, read the entry again and
compare before writing against its current revision.
...
Requires a dedicated `review` credential assigned to a separate agent. Human
review is the default: project policy or explicit human delegation of this exact
candidate revision must authorize its reviewer. The normative permission,
separation, and evidence contract is [Independent agent review](https://github.com/serge-the-hedge/flutte/blob/main/docs/agent-review.md).

A `kind: "candidate"` response contains Source, current target, candidate text,
blank reasons, basis status, authorization, applicable project guidance, and an
opaque `reviewToken`. A `kind: "recordedReview"` response contains `latestReview`:
the recorded decision, actual reviewer, authorization, timestamp, and any final
value fingerprint. It contains no `reviewToken` and requires no new decision.
With current access, recorded results remain readable after candidate
supersession, target removal, or Locale Proposal finalization. Read this URL to
recover a lost write response.

### `POST /candidate-reviews/:revisionId`

Requires the same reviewer identity, current human authorization, and the opaque
token from the corresponding read:

```json
{ "reviewToken": "<from GET>", "decision": { "kind": "accept" } }
```

Rejection uses `{ "kind": "reject", "reason": "Explain the defect" }`.
Acceptance applies exact candidate bytes and any existing Intentional Blank
reason; it cannot supply an edited value. Rejection leaves the live value
unchanged. Acceptance records the actual reviewer and authorizing human decision.

Changes to Source, target, candidate, prior review, guidance, or authority
invalidate the token. Fetch current context and reassess before another decision.
The reviewer credential cannot finalize proposals, build releases, or deliver to
Git. Token revocation blocks future requests. Disabling project review stops
policy-authorized reviews; an active per-revision grant can still authorize its
named reviewer. Completed evidence remains valid after later revocation.
...
Use `POST /translation-tasks` with target
`{ "kind": "newLocale", "localeCode": "pt" }` and scope
`{ "kind": "completeCatalog" }`. Task reads page the exact Source Snapshot;
candidate writes accept at most 16 values or Intentional Blanks. The server owns
the pinned Source evidence, so task callers need not copy fingerprints or Convex
ids. Corrections append immutable revisions. Only human-applied or explicitly
authorized agent-reviewed values can finalize.
...
Prepare and deliver one reviewed catalog at a time; see [Adding languages](https://github.com/serge-the-hedge/flutte/blob/main/docs/adding-languages.md). A later accepted descendant
...
- The current CLI uses `/api/repository-adapter/v1/snapshot-uploads` sessions
  ...
  [transport and artifact contracts](https://github.com/serge-the-hedge/flutte/blob/main/docs/repository-adapter.md).
...
[Repository Adapter transport](https://github.com/serge-the-hedge/flutte/blob/main/docs/repository-adapter.md). Its file-manifest uploads
